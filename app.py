...
def calculate_next_run_time() -> Tuple[int, int]:
    ...
    days_ahead = target_day - now_utc.weekday()
    if days_ahead <= 0:  # Target day already happened this week
        days_ahead += 7
    
    # If today is Saturday and time has passed, schedule for next Saturday
    if days_ahead == 0 and (now_utc.hour * 60 + now_utc.minute) >= (target_hour * 60 + target_minute):
        days_ahead = 7
...
class DiscordBot(discord.Client):
    ...
    def start_scheduler(self):
        ...
        print(f'[SCHEDULER] Next leaderboard run: {datetime.fromtimestamp(timestamp/1000, timezone.utc).strftime("%a, %d %b %Y %H:%M:%S GMT")}')
        print(f'[SCHEDULER] Delay: {round(delay / 1000 / 60)} minutes')
...
async def get_weekly_message_counts(source_channel: discord.TextChannel) -> Dict[str, int]:
    ...
    cutoff_time = datetime.now(timezone.utc) - timedelta(milliseconds=ONE_WEEK_MS)
...
async def run_leaderboard_update(bot: DiscordBot, is_test: bool = False, interaction: discord.Interaction = None):
    ...
    guild_id = interaction.guild_id if interaction else list(bot.guilds)[0].id
    guild = bot.get_guild(guild_id)
    
    if not guild:
        print('[LEADERBOARD] Guild not found.')
        if interaction:
